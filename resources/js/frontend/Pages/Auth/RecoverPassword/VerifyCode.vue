        ...
        <form @submit.prevent="VerifyCodeSubmitHandler" class="login-form">
          <div class="form-group">
            ...
            <div class="verification-code-container">
              <input v-for="(value, index) in codeFields" :key="index" ref="codeInput" class="verification-input"
                type="text" v-model="codeFields[index]" @input="moveFocus(index)"
                @keydown="handleKeydown($event, index)" @paste="handlePaste($event)" maxlength="1" autocomplete="off" />
            </div>
          </div>

          <button class="login-button" type="submit" :disabled="loading || !isCodeComplete">
